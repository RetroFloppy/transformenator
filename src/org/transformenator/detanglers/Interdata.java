...
public class Interdata extends ADetangler
{
	...
	{
		/*
		 * The filesystem is based on 4-byte pointers to "blocks" (256 byte chunks).
      ...
      for (int i = 0; i < 0x5; i ++)
      {
        ...
        if (UnsignedByte.intValue(inData[ixFAT + (i*0x30) + 4]) != 0x00)
        {
          ...
          if (ixFileHead > 0)
          {
            ...
            parent.emitFile(outBinary.toByteArray(), outDirectory, inFile.substring(0,(inFile.lastIndexOf('.')>0?inFile.lastIndexOf('.'):inFile.length())), filename+".bin");
            parent.emitFile(outText.toByteArray(), outDirectory, inFile.substring(0,(inFile.lastIndexOf('.')>0?inFile.lastIndexOf('.'):inFile.length())), filename2);
          }
          else
  ...
  {
    /*
     * Given an index to a 4-byte number, return the number there (ignoring two MSBs)
     */
