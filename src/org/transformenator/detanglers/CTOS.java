...
public class CTOS extends ADetangler
{
      ...
      boolean isDebugMode)
  {
    ...
    lfaFileHeadersBase = UnsignedByte.longValue(inData[(int) lfaWorkingVHB + 78], inData[(int) lfaWorkingVHB + 79],
        inData[(int) lfaWorkingVHB + 80], inData[(int) lfaWorkingVHB + 81]);
    pagesFH = UnsignedByte.intValue(inData[(int) lfaWorkingVHB + 82], inData[(int) lfaWorkingVHB + 83]);
    System.out.println("creation date bytes:     0x" + UnsignedByte.toString(inData[(int) lfaWorkingVHB + 54])
        + UnsignedByte.toString(inData[(int) lfaWorkingVHB + 55])
        + UnsignedByte.toString(inData[(int) lfaWorkingVHB + 56])
        + UnsignedByte.toString(inData[(int) lfaWorkingVHB + 57]));
    System.out.println("modification date bytes: 0x" + UnsignedByte.toString(inData[(int) lfaWorkingVHB + 58])
        + UnsignedByte.toString(inData[(int) lfaWorkingVHB + 59])
        + UnsignedByte.toString(inData[(int) lfaWorkingVHB + 60])
        + UnsignedByte.toString(inData[(int) lfaWorkingVHB + 61]));
    ...
    for (int i = 0; i < pagesFH; i++)
    {
      ...
      if (filenameLength > 0)
      {
        ...
        int fileLength = (int) UnsignedByte.longValue(inData[newBase + 111], inData[newBase + 112],
            inData[newBase + 113], inData[newBase + 114]);
        ...
        for (int j = 0; j < 32; j++)
        {
          ...
          int extentLen = (int) UnsignedByte.longValue(inData[newBase + 249 + (j * 4)], inData[newBase + 250 + (j * 4)],
              inData[newBase + 251 + (j * 4)], inData[newBase + 252 + (j * 4)]);
          if (extentLen != 0)
          {
            // This extent is in use... address it
            int lfaExtent = (int) UnsignedByte.longValue(inData[newBase + 121 + (j * 4)],
                inData[newBase + 122 + (j * 4)], inData[newBase + 123 + (j * 4)], inData[newBase + 124 + (j * 4)]);
            int copyLength = 0;
        ...
        parent.emitFile(out.toByteArray(), outDirectory,
            ...
            fileName + fileSuffix, modificationDate);
      }
    }
  }
