      ...
      boolean isDebugMode)
  {
    ...
    switch (EbcdicUtil.asciiChar(inData[volOffset + 0x4b]))
    {
      ...
      case '3':
        ...
        break;
      default:
        System.err.println("ERROR: Unable to determine sector size; sector identifier is ["
            + EbcdicUtil.asciiChar(inData[volOffset + 0x4b]) + "] (0x"
            + Integer.toHexString(UnsignedByte.intValue(inData[volOffset + 0x4b])) + ")");
        break;
    }
    switch (EbcdicUtil.asciiChar(inData[volOffset + 0x47]))
    {
      case ' ':
        indexCylinderSize = 3328;
        headSize = 0;
        if (DEBUG)
      ...
      case 0xb6:
        ...
        break;
      default:
        System.err.println("ERROR: Unable to determine disk density; density identifier is ["
            + EbcdicUtil.asciiChar(inData[volOffset + 0x47]) + "] (0x"
            + Integer.toHexString(UnsignedByte.intValue(inData[volOffset + 0x47])) + ")");
        break;
    }
    if (inData.length == 1258496) // AS/400 / System/36 disks
    {
    ...
    if ((sectorSize > 0) && (cylinderSize > 0) && (indexCylinderSize > 0))
    {
      if (DEBUG)
        System.err
            .println(", Sector size: " + sectorSize + " Sectors/Cyl: " + cylinderSize / ((headSize + 1) * sectorSize)
                + " Cylinder size: " + cylinderSize + " Index Cyl size: " + indexCylinderSize);
      ...
      for (int i = 0; i < indexCylinderSize - directoryOffset; i += 128)
      {
        ...
        if (EbcdicUtil.asciiChar(inData[directoryOffset + i]) != 'D')
        {
          ...
          try
          {
            ...
            fileStart = indexCylinderSize + ((startCyl - 1) * cylinderSize) + (startHead * cylinderSize / 2)
                + ((startSector - 1) * sectorSize);
            int fileEnd = sectorSize + indexCylinderSize + ((endCyl - 1) * cylinderSize) + (endHead * cylinderSize / 2)
                + ((endSector - 1) * sectorSize);
            if (DEBUG)
            {
              ...
              try
              {
                ...
                if (fileStart + fileLength <= inData.length)
                {
                  ...
                  */
                  ...
                  System.err.println(
                      "Error: file " + fileName + fileSuffix + " would exceed the capacity of the disk image.");
              }
