...
{
  @Override
  public void detangle(FileInterpreter interpreter, byte[] inData, String outDirectory, String inFile,
      String fileSuffix, boolean isDebugMode)
  {
    ...
    if (offset + 4 < inData.length)
    {
      ...
      if (!ec.equals("----"))
      {
        if ((debugLevel == 1) || (debugLevel == 2))
        {
          ...
          System.err.println(" with length: 0x" + Integer
              .toHexString(
                  0x10000 | UnsignedByte.intValue(inData[offset + 0]) * 256 + UnsignedByte.intValue(inData[offset + 1]))
              .substring(1));
        }
        if (ec.equals("DEXT (0xe0)"))
        {
          ...
          for (int q = 4; q < recLen; q += 4)
          {
            ...
            if ((newRec < inData.length) && (newRec > 0))
            {
              if (debugLevel > 0)
              {
                System.err.print("0x" + Integer.toHexString(0x1000000 | offset + q).substring(1) + ": Pointer: 0x"
                    + Integer.toHexString(0x1000000 | newRec).substring(1) + ": ");
                if (newRec == locEHL1)
          ...
          if (ec.equals("EHL1 (0x20)"))
          {
            // Dig out the EHL1 length, return it when we pop back out
            ret = UnsignedByte.intValue(inData[offset + 0x10]) * 65536
                + UnsignedByte.intValue(inData[offset + 0x11]) * 256 + UnsignedByte.intValue(inData[offset + 0x12]);
            if (dive)
              processRecord(inData, outDirectory, inFile, offset + recLen, dive, debugLevel);
          }
          else
            if (ec.equals("NAME (0x80)"))
            {
              String newName = EbcdicUtil.toAscii(inData, offset + 4, 44).trim().replace("\\", "-").replace("/", "-")
                  .replace("?", "-");
              if (debugLevel > 0)
  ...
  public static String getRecordEyecatcher(byte inData[], int offset)
  {
    ...
    if (offset + 3 < inData.length)
    {
      ...
      if ((recTypeHi == 0x20) && (recTypeLo == 0x00) && (recLength == 0x19))
        ...
        if ((recTypeHi == 0x40) && (recTypeLo == 0x00))
          recordEyecatcher = "ABM  (0x40)"; // Allocation bitmap
        else
          if ((recTypeHi == 0x60) && (recTypeLo == 0x00))
