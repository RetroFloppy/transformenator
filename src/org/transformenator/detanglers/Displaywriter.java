...
public class Displaywriter extends ADetangler
{
	...
	public void detangle(FileInterpreter interpreter, byte[] inData, String inFile, String outDirectory, String fileSuffix)
	{
		...
		if ((clippedImage != null) && (locEHL1 > -1))
		{
			...
			while (!done)
			{
				...
				{
					processRecord(clippedImage, offset, true, debugLevel);
				}
				if (offset > total)
				{
					done = true;
	...
	public int processRecord(byte inData[], int offset, boolean dive, int debugLevel)
	{
		...
		if (!ec.equals("----"))
		{
				...
				for (int q = 4; q < recLen; q += 4)
				{
					...
					if ((newRec < inData.length) && (newRec > 0))
					{
						...
						if ((dive) && (newRec != locEHL1))
							processRecord(inData, newRec, true, debugLevel);
					}
				}
				// Note - DEXT entries will not have others following it in the same sector
			}
			else if (ec.equals("EHL1 (0x20)"))
			{
				...
				if (dive)
					processRecord(inData, offset + recLen, dive, debugLevel);
			}
	...
	public int dumpBareRecord(byte inData[], int offset, int debugLevel)
	{
		String ec = getRecordEyecatcher(inData, offset);
		int recLen = UnsignedByte.intValue(inData[offset + 0]) * 256 + UnsignedByte.intValue(inData[offset + 1]);
		if (!ec.equals("----"))
		{
			if ((debugLevel == 2) || ((debugLevel == 3) && (ec.equals("TEXT (0xe8)"))))
				processRecord(inData, offset, false, debugLevel);
			offset += recLen;
		}
		else
		{
